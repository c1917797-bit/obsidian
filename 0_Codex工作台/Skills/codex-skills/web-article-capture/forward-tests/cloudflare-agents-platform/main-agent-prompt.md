# Main Agent Prompt: Run Cloudflare Agents Platform Web Capture Forward Test
...
Paste the URLs from `forward-tests/cloudflare-agents-platform/candidate/input/urls.txt` into the `<assigned URLs>` slot. Do not include this file path in the child prompt.
...
输出目录：
.tmp/forward-tests/web-article-capture/cloudflare-agents-platform/<run-id>/
