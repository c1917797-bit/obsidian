# Main Agent Prompt: Run Tair KVCache Simulation Web Capture Forward Test
...
Paste the URLs from `forward-tests/tair-kvcache-simulation/candidate/input/urls.txt` into the `<assigned URLs>` slot. Do not include this file path in the child prompt.
...
输出目录：
.tmp/forward-tests/web-article-capture/tair-kvcache-simulation/<run-id>/
