...
def print_image_scale_errors(issues: list[dict[str, object]]) -> None:
    print("[ERROR] html-ppt render QA failed: slide images are scaled below 80%.")
    for issue in issues:
    ...
    print(
        "    React: Keep the original image near its original size, split dense figures across pages, "
...
def clean_png_outputs(out_path: Path, stem: str, count: int) -> int:
    if count <= 1 or not out_path.exists():
        return 0
    removed = 0
    for png in out_path.glob(f"{stem}_*.png"):
        png.unlink()
        removed += 1
    if removed:
...
def write_manifest(
    ...
    manifest = {
        ...
        "note": (
            ...
            if succeeded
            else "Render completed screenshots but failed QA checks; fix HTML and rerun."
        ),
    }
...
def prepare_output_location(out_path: Path, count: int) -> None:
    if count > 1 or not out_path.suffix:
...
def render(html_file: Path, count_arg: str, out_arg: str | None, width: int, height: int, timeout_ms: int) -> int:
    ...
    prepare_output_location(out_path, count)
    if count > 1:
        clean_png_outputs(out_path, stem, count)
    ...
    failed = bool(image_scale_issues or navigation_issues)
    write_manifest(out_path, html_file, count, width, height, png_paths, "failed" if failed else "succeeded")
    if not failed:
        ...
        print(f"failed: rendered {count} slide(s) from {html_file}, but QA checks failed.")
    if image_scale_issues:
        return 1
...
def run_self_test() -> list[str]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        root = Path(temp_dir)
        single_out_dir = root / "fresh-single-export"
        prepare_output_location(single_out_dir, 1)
        if not single_out_dir.is_dir():
            errors.append("single-slide directory output was not created")
        single_out_file = root / "nested" / "slide.png"
        prepare_output_location(single_out_file, 1)
        if not single_out_file.parent.is_dir():
            errors.append("single-slide file parent output was not created")
        multi_out_dir = root / "multi-export"
        multi_out_dir.mkdir()
        keep_png = multi_out_dir / "keep.png"
        stale_png = multi_out_dir / "deck_01.png"
        keep_png.write_text("keep", encoding="utf-8")
        stale_png.write_text("stale", encoding="utf-8")
        removed = clean_png_outputs(multi_out_dir, "deck", 2)
        if removed != 1 or not keep_png.exists() or stale_png.exists():
            errors.append("multi-slide cleanup should only remove current deck PNGs")
    return errors
