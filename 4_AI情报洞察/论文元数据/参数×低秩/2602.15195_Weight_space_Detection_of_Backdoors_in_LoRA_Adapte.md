---
type: literature-corpus-entry
citekey: merenciano2026weight
title: "Weight space Detection of Backdoors in LoRA Adapters"
authors:
  - "David Puertolas Merenciano"
  - "Ekaterina Vasyagina"
  - "Kevin Zhu"
  - "Javier Ferrando"
  - "Maheep Chaudhary"
year: 2026
venue: arXiv
venue_type: conference
arxiv_id: "2602.15195"
doi: ""
url: "https://arxiv.org/abs/2602.15195"
pdf_url: "https://arxiv.org/pdf/2602.15195"
object: 参数
method: 低秩
cell: "参数×低秩"
all_objects:
  - "参数"
all_methods:
  - "低秩"
abstract: |
  LoRA adapters let users fine-tune large language models (LLMs) efficiently. However, LoRA adapters are shared through open repositories like Hugging Face Hub \citep{huggingface_hub_docs}, making them vulnerable to backdoor attacks. Current detection methods require running the model with test input data -- making them impractical for screening thousands of adapters where the trigger for backdoor behavior is unknown. We detect poisoned adapters by analyzing their weight matrices directly, without running the model -- making our method trigger-agnostic. For each attention projection (Q, K, V, O)
key_innovation: "提出了一种无需运行模型即可通过直接分析权重矩阵来检测LoRA适配器中的后门的方法。"
performance: "未提数字"
code_url: ""
hf_model: ""
status: unread
priority: P1
date_added: 2026-07-06
obtained_via: "deep_collect_v2_supp"
obtained_at: 2026-07-06
tags:
  - inference-compression
  - 参数
  - 低秩
  - merenciano26
---

# Weight space Detection of Backdoors in LoRA Adapters

## 基本信息

| 字段 | 值 |
|------|------|
| **Citekey** | `merenciano2026weight` |
| **年份** | 2026 |
| **会议/期刊** | arXiv |
| **arXiv** | [2602.15195](https://arxiv.org/abs/2602.15195) |
| **4×5分类** | 参数×低秩 |
| **来源** | deep_collect_v2_supp |
| **作者** | David Puertolas Merenciano, Ekaterina Vasyagina, Kevin Zhu |

## 摘要

LoRA adapters let users fine-tune large language models (LLMs) efficiently. However, LoRA adapters are shared through open repositories like Hugging Face Hub \citep{huggingface_hub_docs}, making them vulnerable to backdoor attacks. Current detection methods require running the model with test input data -- making them impractical for screening thousands of adapters where the trigger for backdoor behavior is unknown. We detect poisoned adapters by analyzing their weight matrices directly, without running the model -- making our method trigger-agnostic. For each attention projection (Q, K, V, O)

## 关键创新

*待精读后在此填写 1-3 个方法核心创新点*

> 提示：
> - 这个方法解决什么问题？
> - 相比已有方法的本质区别？
> - 实验中验证了什么关键指标？

## 性能数据

| 模型/场景 | 压缩率 | 精度 | 加速比 | 显存 |
|-----------|--------|------|--------|------|
| 待填 | | | | |

> 精读时把论文表格中的关键数字搬过来

## 代码与资源

- **PDF**: https://arxiv.org/pdf/2602.15195
- **代码**: （精读时查找GitHub链接）
- **HuggingFace模型**: （如适用）

## 我的笔记

*精读笔记在此写，或链接到 [[专题笔记名]]*

## 相关论文

*精读时用Obsidian双链 `[[arxiv_id]]` 连接相关工作*

---

*基于 ARS v3.6.4 literature_corpus_entry schema*  
*生成日期: 2026-07-06 | 来源: deep_collect_v2_supp*