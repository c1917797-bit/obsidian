# Judge Rubric: Cloudflare Agents Platform Web Capture
...
- Content Selection: captures the smallest article/main正文 scope from Cloudflare Agents documentation, with a clean terminal boundary before post-body modules or page chrome.
...
- Source Notes: records concise capture notes inside `source.md` when capture mode, source boundaries, tail audit, missing/rejected/uncertain media, Markdown variants, fallback source, or reliability details matter.
