...
- Content Selection: captures the smallest article/main正文 scope from NVIDIA Dynamo docs and technical blog sources, with a clean terminal boundary before post-body modules or page chrome.
...
- Source Notes: records concise capture notes inside `source.md` when capture mode, source boundaries, tail audit, missing/rejected/uncertain media, versioned docs, fallback source, or reliability details matter.
