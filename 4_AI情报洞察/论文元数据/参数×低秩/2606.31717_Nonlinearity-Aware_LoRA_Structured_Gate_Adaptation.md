---
type: literature-corpus-entry
citekey: yuan2026nonlinearity
title: "Nonlinearity-Aware LoRA: Structured Gate Adaptation under Low-Rank Constraints"
authors:
  - "Shuai Yuan"
  - "Sudong Cai"
  - "Bingzhi Chen"
  - "Shuyuan Zheng"
  - "Chuan Xiao"
year: 2026
venue: arXiv
venue_type: conference
arxiv_id: "2606.31717"
doi: ""
url: "https://arxiv.org/abs/2606.31717"
pdf_url: "https://arxiv.org/pdf/2606.31717"
object: 参数
method: 低秩
cell: "参数×低秩"
all_objects:
  - "参数"
  - "激活"
all_methods:
  - "低秩"
abstract: |
  Low-rank adaptation (LoRA) is commonly viewed as an update-space approximation to full fine-tuning, yet this view is incomplete for self-gated Transformer feed-forward networks. In gated FFNs, a low-rank residual can change not only projected features but also the nonlinear selection weights that determine which channels contribute to the output. We formalize this effect as selection misalignment and connect it to the local effective homogeneity of self-gated activations. This motivates a nonlinearity-aware principle for parameter-efficient fine-tuning: low-rank updates should allocate capacit
key_innovation: "提出了非线性感知LoRA方法，通过结构化门控适应在低秩约束下优化自门控Transformer前馈网络。"
performance: "未提数字"
code_url: "未开源"
hf_model: ""
status: unread
priority: P1
date_added: 2026-07-06
obtained_via: "deep_collect_v2_supp"
obtained_at: 2026-07-06
tags:
  - inference-compression
  - 参数
  - 低秩
  - yuan26
---

# Nonlinearity-Aware LoRA: Structured Gate Adaptation under Low-Rank Constraints

## 基本信息

| 字段 | 值 |
|------|------|
| **Citekey** | `yuan2026nonlinearity` |
| **年份** | 2026 |
| **会议/期刊** | arXiv |
| **arXiv** | [2606.31717](https://arxiv.org/abs/2606.31717) |
| **4×5分类** | 参数×低秩 |
| **来源** | deep_collect_v2_supp |
| **作者** | Shuai Yuan, Sudong Cai, Bingzhi Chen |

## 摘要

Low-rank adaptation (LoRA) is commonly viewed as an update-space approximation to full fine-tuning, yet this view is incomplete for self-gated Transformer feed-forward networks. In gated FFNs, a low-rank residual can change not only projected features but also the nonlinear selection weights that determine which channels contribute to the output. We formalize this effect as selection misalignment and connect it to the local effective homogeneity of self-gated activations. This motivates a nonlinearity-aware principle for parameter-efficient fine-tuning: low-rank updates should allocate capacit

## 关键创新

*待精读后在此填写 1-3 个方法核心创新点*

> 提示：
> - 这个方法解决什么问题？
> - 相比已有方法的本质区别？
> - 实验中验证了什么关键指标？

## 性能数据

| 模型/场景 | 压缩率 | 精度 | 加速比 | 显存 |
|-----------|--------|------|--------|------|
| 待填 | | | | |

> 精读时把论文表格中的关键数字搬过来

## 代码与资源

- **PDF**: https://arxiv.org/pdf/2606.31717
- **代码**: （精读时查找GitHub链接）
- **HuggingFace模型**: （如适用）

## 我的笔记

*精读笔记在此写，或链接到 [[专题笔记名]]*

## 相关论文

*精读时用Obsidian双链 `[[arxiv_id]]` 连接相关工作*

---

*基于 ARS v3.6.4 literature_corpus_entry schema*  
*生成日期: 2026-07-06 | 来源: deep_collect_v2_supp*