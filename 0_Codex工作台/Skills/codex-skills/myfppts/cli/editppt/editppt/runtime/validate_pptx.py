...
def is_contract_supplied_asset(entry):
    """Allow original user/official assets named by the slide content contract.

    The upstream image-reconstruction workflow correctly rejects a raster crop
    taken from the flattened source page. This combined workflow also receives
    original screenshots and photos through deck_spec.json; those assets should
    remain independent replaceable image objects instead of being re-generated
    or extracted from the design draft.
    """
    return entry.get("source_type") == "user-provided" and entry.get("original_asset") is True
...
def foreground_asset_contract_violations(manifest):
    ...
    for index, item in enumerate(manifest.get("visual_inventory", [])):
        ...
        if path:
            ...
            if source_type in {"user-provided", "user-approved-rasterization"} and not is_contract_supplied_asset(provenance):
                violations.append(
                    {
                        "field": field,
                        "path": path,
    ...
    for index, entry in enumerate(manifest.get("asset_provenance", [])):
        ...
        if (
            source_type in {"user-provided", "user-approved-rasterization"}
            and not is_contract_supplied_asset(entry)
            and contains_any(
